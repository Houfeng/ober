...
export class Observer extends EventEmitter {
  ...
  public set(name: string, value: any) {
    ...
    Object.defineProperty(this.target, name, {
      get() {
        ...
        if (Observer.state) observer.emitGet({ name, value });
      ...
      set(value) {
        ...
        observer.emitChange({ name, value });
      },
      configurable: true,
      enumerable: true
    });
  ...
  addChild(child: Observer, name: string) {
    ...
    child.parents.push({ parent: this, name });
  }
  ...
  dispatch(eventName: string, event: IObserveEvent) {
    if (event.src === this || !Observer.state) return;
    ...
    this.parents.forEach(item => {
      ...
      parentEvent.path = isNull(event.path)
        ? item.name
